                ...
                <div class="col">
                    <div class="page-title">
                        <ul class="breadcrumb mb-0">
                            ...
                            @if($category)
                                <li class="breadcrumb-item"><a href="{{ route('category.products', [$category->id, Str::slug($category->title)]) }}">{{ $category->title }}</a></li>
                            @endif
            ...
            <div class="row learts-mb-n40">
                ...
                <div class="col-lg-6 col-12" data-aos="fade-right">
                    <div class="product-images">
                        {{-- $gallery is passed in directly from PageController@single_product --}}

                        <div class="pd-main-image mb-3" style="position:relative;">
                            <img id="pdMainImage" src="{{ asset('images/product/' . $gallery->first()) }}" alt="{{ $product->title }}">
                        ...
                        <div class="pd-thumb-carousel">
                            ...
                            <div class="pd-thumb-track" id="pdThumbRow">
                                @foreach($gallery as $g)
                                    <div class="pd-gallery-thumb {{ $loop->first ? 'active-thumb' : '' }}" data-image="{{ asset('images/product/' . $g) }}">
                                        <img src="{{ asset('images/product/' . $g) }}" alt="{{ $product->title }} thumbnail {{ $loop->iteration }}">
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="pd-share-row mt-3">
                            ...
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank" title="Facebook"><i class="fab fa-facebook-f"></i></a>
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($product->title) }}" target="_blank" title="Twitter"><i class="fab fa-twitter"></i></a>
                            <a href="mailto:?subject={{ urlencode($product->title) }}&body={{ urlencode(url()->current()) }}" title="Email"><i class="fa fa-envelope"></i></a>
                            <a href="https://pinterest.com/pin/create/button/?url={{ urlencode(url()->current()) }}&media={{ urlencode(asset('images/product/'.$gallery->first())) }}&description={{ urlencode($product->title) }}" target="_blank" title="Pinterest"><i class="fab fa-pinterest-p"></i></a>
                            <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}" target="_blank" title="LinkedIn"><i class="fab fa-linkedin-in"></i></a>
                            <a href="https://wa.me/?text={{ urlencode($product->title . ' - ' . url()->current()) }}" target="_blank" title="WhatsApp"><i class="fab fa-whatsapp"></i></a>
                            <a href="https://t.me/share/url?url={{ urlencode(url()->current()) }}&text={{ urlencode($product->title) }}" target="_blank" title="Telegram"><i class="fab fa-telegram-plane"></i></a>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-6 col-12 learts-mb-40" data-aos="fade-left">
                    <div class="product-summery">
                        ...
                        <div class="pd-price-row" id="pdPriceRow">
                            @if($hasDiscount)
                                <span class="pd-price-new" id="pdPriceNew">{{ env('CURRENCY') }}{{ $product->discount_price }}</span>
                                <span class="pd-price-old" id="pdPriceOld">{{ env('CURRENCY') }}{{ $product->price }}</span>
                            @else
                                <span class="pd-price-new" id="pdPriceNew">{{ env('CURRENCY') }}{{ $product->price }}</span>
                        ...
                        <div class="pd-size-row mb-3">
                            ...
                            <div class="pd-size-pills">
                                @foreach($product->variation as $v)
                                    <button type="button"
                                        class="pd-size-pill {{ $loop->first ? 'active' : '' }}"
                                        data-variation-id="{{ $v->id }}"
                                        data-price="{{ $v->price }}"
                                        data-qty="{{ $v->qty }}">{{ $v->variant }}</button>
                                @endforeach
                            </div>
                            <input type="hidden" id="selected_variation_id" value="{{ $product->variation->first()->id }}">
                        </div>
                        <style>
                            ...
                            document.addEventListener('DOMContentLoaded', function () {
                                ...
                                pills.forEach(function (pill) {
                                    pill.addEventListener('click', function () {
                                        ...
                                        if (priceNew) { priceNew.textContent = '{{ env("CURRENCY") }}' + price; }
                        ...
                        @elseif($available_stock <= 5)
                            <p class="pd-stock low-stock" id="pdStockBox"><i class="fa fa-exclamation-circle"></i> Only {{ $available_stock }} left in stock - order soon</p>
                        @else
                            <p class="pd-stock in-stock" id="pdStockBox"><i class="fa fa-check-circle"></i> In Stock</p>
                        @endif
                        ...
                        <div class="product-buttons mt-4">
                            <button type="button" class="btn xm-btn-buy-now mb-2" onclick="buyNow({{ $product->id }})" {{ (!is_null($available_stock) && $available_stock <= 0) ? 'disabled' : '' }}>
                                BUY NOW
                            </button>
                            <div class="row-actions" style="display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:center;">
                                <div class="xm-qty product-quantity" style="display:inline-flex;">
                                    ...
                                    <input type="text" id="qty" class="input-qty" value="1" data-max="{{ $available_stock }}" {{ (!is_null($available_stock) && $available_stock <= 0) ? 'disabled' : '' }} style="width:52px;height:44px;text-align:center;border:1px solid #E8E0D8;border-left:0;border-right:0;">
                                ...
                                <button type="button" class="btn xm-btn-add-cart w-100 added_to_cart_{{ $product->id }}"
                                    onclick="addToCart({{ $product->id }})" {{ (!is_null($available_stock) && $available_stock <= 0) ? 'disabled' : '' }}>
                                    ADD TO CART
                                </button>
                            </div>
                        </div>
                                ...
                                <div class="scroll">
                                    <div class="product-thumb">
                                        <span class="product-badges">
                                            @if($sp->is_sale == 1 && $sp->discount_price > 0)
                                                <span class="onsale">Sale</span>
                                            @endif
                                        </span>
                                        <a href="{{ route('single.product', [$sp->id, Str::slug($sp->title)]) }}" class="image">
                                            <img class="rounded" src="{{ asset('images/product/'. $sp->image) }}" alt="{{ $sp->title }}">
                                            @if($sp->hover_image)
                                                <img class="image-hover rounded border" src="{{ asset('images/product/'. $sp->hover_image) }}" alt="{{ $sp->title }}">
                                            @endif
                                        </a>
                                    </div>
                                    <div class="product-info">
                                        <h6 class="title"><a href="{{ route('single.product', [$sp->id, Str::slug($sp->title)]) }}">{{ $sp->title }}</a></h6>
                                        <span class="price">
                                            @if($sp->is_sale == 1 && $sp->discount_price > 0)
                                                <span class="old">{{ env('CURRENCY') }}{{ $sp->price }}</span>
                                                <span class="new">{{ env('CURRENCY') }}{{ $sp->discount_price }}</span>
                                            @else
                                                <span class="new">{{ env('CURRENCY') }}{{ $sp->price }}</span>
                                            @endif
                                        </span>
                                        <a href="{{ route('single.product', [$sp->id, Str::slug($sp->title)]) }}" class="btn btn-order-now">
                                            Order Now
                                        </a>
                                    </div>
                                </div>
